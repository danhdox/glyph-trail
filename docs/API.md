...
interface GlyphTrailSettings {
  ...
  glitch: {
    intensity: number; // 0-100 amount of RGB-split + jitter as the cursor sweeps through
    speed: number; // 0-100 how easily cursor motion triggers the glitch (sensitivity)
  };
}
```
